...
import { ditchNetworkById, fetchNetworkById } from '../../../features/networkCreate/networkSlice'; // Adjust path if needed
...
const DitchNetworkPageMobile = () => {
  ...
  const { data: networkData } = useSelector((state) => state.network.currentNetwork);
  const { status, error } = useSelector((state) => state.network.ditchStatus);

  // Fetch the network data if it's not already loaded
  useEffect(() => {
    if (!networkData && id) {
      dispatch(fetchNetworkById(id));
    }
  }, [networkData, id, dispatch]);
  ...
  const isButtonDisabled = !networkData || inputName.trim() !== networkData.name;
    ...
    <div className="min-h-screen bg-black text-white p-4 font-sans flex flex-col">
      ...
      <div className="max-w-md">
        ...
        <p className="text-sm text-gray-400 leading-relaxed mb-8">
          This action is irreversible. To confirm, please type the full name of the network: 
        ...
        <button 
          ...
          className="w-full bg-red-800/80 text-white px-6 py-3 rounded-lg text-sm font-semibold hover:bg-red-700 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed"
