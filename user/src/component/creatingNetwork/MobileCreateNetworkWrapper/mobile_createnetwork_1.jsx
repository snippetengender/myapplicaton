...
import { setFormData, checkNetworkName } from '../../../features/networkCreate/networkSlice';
  ...
  return (
    <div className="bg-black text-white min-h-screen flex flex-col p-6 pb-10">
      ...
      <header className="flex items-center justify-between w-full">
        ...
        <div className="bg-neutral-800 text-xs rounded-full px-3 py-1.5">1/3</div>
      </header>

      {/* Main Content */}
      <main className="flex-grow w-full pt-4">
        <h1 className="text-2xl font-semibold mb-2">About your Network</h1>
        <p className="text-xs text-neutral-400 mb-6">Start with a name and a short description to tell people what your network is about.</p>
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-baseline mt-1">
            ...
            <span className="text-xs whitespace-nowrap ml-4">#15 character only</span>
          </div>
        </div>
        ...
        <div className="mt-4 space-y-1">
          ...
          />
          <div className="px-1">
            <p className="text-left text-sm text-neutral-500">{description.length}/500</p>
            {/* Dynamic error/status message */}
            {nameCheck.status === 'loading' && <p className="text-yellow-500 text-sm mt-1">Checking name...</p>}
            {nameCheck.status === 'succeeded' && !nameCheck.isAvailable && name.length > 0 && <p className="text-red-500 text-sm mt-1">Sorry, this network name is already taken.</p>}
            {nameCheck.status === 'succeeded' && nameCheck.isAvailable && name.length > 0 && <p className="text-green-500 text-sm mt-1">Name is available!</p>}
          </div>
        </div>
      </main>
